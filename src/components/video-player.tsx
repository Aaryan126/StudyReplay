...
export function VideoPlayer({ title, durationSeconds = 0, selectedTimeSec = 0 }: VideoPlayerProps) {
  const progress = durationSeconds > 0 ? Math.min(100, (selectedTimeSec / durationSeconds) * 100) : 0;
  ...
  return (
    <section aria-label="Video player" className="overflow-hidden rounded-2xl border border-[#26362d] bg-[#17231d] shadow-[0_18px_50px_rgba(23,35,29,0.16)]">
      ...
      <div className="flex items-center gap-3 border-t border-white/10 px-4 py-3 text-xs text-white/65">
        <span aria-label="Current video timestamp">{formatTimestamp(selectedTimeSec)}</span>
        <div className="h-1 flex-1 rounded-full bg-white/15">
          <div className="h-1 rounded-full bg-[#70c698] transition-[width]" style={{ width: `${progress}%` }} />
        </div>
